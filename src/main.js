...
function buildChartDataFromJson(json) {
  ...
  for (let h=0; h<24; h++) {
    let record = json[h]; // %3 to start at 3AM
...
function switchToHourlyView(index) {
  ...
  if (index==0) {
    mymap.setPaintProperty('taz','fill-extrusion-color',
      {property: 'trips',stops: colorRampDaily});
  } else {
    mymap.setPaintProperty('taz','fill-extrusion-color',
      {property: 'trips',stops: colorRampHourly});
  }
}
...
function buildDailyDetails(json) {
    ...
    for (let record of json) {
      ...
      cachedHourlyData[day][time_index]['dropoffs'][taz] = dropoff;
      cachedHourlyData[day][time_index]['pickups'][taz] = pickup;
