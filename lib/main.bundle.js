  ...
  try {
    for (var _iterator = tazs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      if (taz.taz in tripTotals) {
        var mine = tripTotals[parseInt(taz.taz)][day];
...
function updateColors() {
  for (var taz in tazLayers) {
    var shade = '#222';
    var layer = tazLayers[taz];

    var tazData = tripTotals[parseInt(taz)];
    if (tazData) {
      var dayData = tazData[day];
